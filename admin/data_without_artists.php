...
<div class="wrap">
    ...
    if (isset($_POST['chosen'])) {
        ...
        if (isset($_POST['monthly'])) {
            ...
            $data_without_artists_monthly = $wpdb->get_results($wpdb->prepare('SELECT * FROM wp_mokkamerch_BILL_DATA_SUMMARY_JOINED_ARTISTS_PERM_TABLE WHERE DATE(order_sold_at) BETWEEN %s AND %s AND (artist_name is NULL OR "")', $abrechnungsBeginn, $abrechnungsEnde),ARRAY_A);


            echo "<pre>";
            print_r($data_without_artists_monthly);
            echo "</pre>";
        } elseif (isset($_POST['all_time'])) {
            echo "<h1>All time Summary</h1>";
            $data_without_artists = $wpdb->get_results('SELECT * FROM wp_mokkamerch_BILL_DATA_SUMMARY_JOINED_ARTISTS_PERM_TABLE WHERE (artist_name is NULL OR "")', ARRAY_A);
            echo "<pre>";
            print_r($data_without_artists);
            echo "</pre>";
        }
